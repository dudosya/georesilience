...
def test_cascade_terminates_and_is_monotonic(g: nx.Graph, seed: int) -> None:
    ...
    cfg = CascadeConfig(alpha=0.2, initial_failures=1, mode=FailureMode.random, seed=seed, max_steps=30)
    ...
    # monotonic: once failed, stays failed (implied by single final table). We can ensure failed_step is unique.
    failed_steps = result.nodes.filter(pl.col("failed") == True)["failed_step"].to_list()
...
def test_determinism_same_seed() -> None:
    g = nx.path_graph(10)
    metrics = compute_node_metrics(g)
    cfg = CascadeConfig(alpha=0.2, initial_failures=2, mode=FailureMode.random, seed=123, max_steps=50)
